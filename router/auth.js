...
router.post('/coursedata', async(req, res) => {
    const { title, description, lessons, duration, certificates, price, img, offer } = req.body;

    if (!title || !description || !lessons || !duration || !certificates || !price || !img || !offer) {
        return res.status(400).json({ error: 'Please provide all required field' });
    }
    ...
    try {
        ...
        const User = new Course({ title, description, lessons, duration, certificates, price, img, offer });
...
router.post('/peeps', async(req, res) => {
    ...
    try {
        ...
        const User = new Peeps({ name, jobTitle, description, image, placedImage }); // Use Peeps model here
...
router.get('/peeps', async(req, res) => {
    try {
        // Retrieve all Peeps data from the database
        const peepsData = await Peeps.find();

        // Check if any Peeps data were found
        if (!peepsData || peepsData.length === 0) {
            return res.status(404).json({ error: 'No Peeps data found' });
        }

        // If Peeps data were found, send them in the response
        res.status(200).json(peepsData);
    } catch (err) {
        console.error(err);
        res.status(500).json({ error: "Failed to fetch Peeps data" });
    }
});
